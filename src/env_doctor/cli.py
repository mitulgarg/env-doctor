...
def cuda_install_command(cuda_version: str = None, output_json: bool = False,
    ...
    if cuda_version:
        ...
        print(f"\nDriver: {driver_version} (supports up to CUDA {max_cuda})")
...
# GitHub Actions workflow generated by env-doctor
# Validates GPU driver, CUDA toolkit, and Python library compatibility
# Edit triggers, Python version, or steps to fit your project
...
jobs:
  validate-env:
    runs-on: ubuntu-latest  # Change to [self-hosted, gpu] for GPU runners
...
def init_command(github_actions=False, force=False):
    """Generate CI/CD integration files."""
    if not github_actions:
        print("Usage: env-doctor init --github-actions")
        print()
        print("Available integrations:")
        print("  --github-actions  Generate GitHub Actions workflow file")
        sys.exit(0)

    workflow_dir = os.path.join(".github", "workflows")
    workflow_path = os.path.join(workflow_dir, "env-doctor.yml")

    if os.path.exists(workflow_path) and not force:
        print(f"File already exists: {workflow_path}")
    ...
    print()
    print("Next steps:")
    print(f"  1. Review and edit {workflow_path} to fit your project")
    print("  2. Commit and push to activate the workflow")
    print()
    print("Common customizations:")
    print("  - Change Python version to match your project")
    print("  - Change runner to [self-hosted, gpu] for GPU validation")
...
Examples:
  ...
  env-doctor report install     # Set up periodic reporting to a dashboard
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Generate CI/CD integration files"
    )
